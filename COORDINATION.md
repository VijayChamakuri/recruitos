...
| Claude Code | b/core-hard-requirements | T9 part 2, step 4 of 6: three-valued hard requirement resolution in `packages/core/src/pipeline/`. Then routing and proposals, one PR each. | ready PR |
| Antigravity | c/runtime-read-models | Named bounded read use-cases in `packages/runtime/src/read-models/` (`listCandidates`, `listResolutionTasks`, `readCandidatePacket` scaffold) | ready PR #34 |
