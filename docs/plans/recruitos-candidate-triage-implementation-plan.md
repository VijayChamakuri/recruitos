...
The first `@recruitos/runtime` slice contains the pinned SQLite and Drizzle dependencies, a typed connection lifecycle, verified connection PRAGMAs, and a neutral strict migration-smoke table. The next runtime slice adds a generic short-transaction helper, neutral command envelopes and receipts, successful replay protection, and expected-version conflict handling. Failed-command receipt writing and audit persistence remain deferred to their shared future boundary. Product-specific schema and migrations, applications, CI, rubric-specific implementation, product fixtures, eval implementation, product tests, and product UI do not exist yet. T1, T2, T3, and T4 remain in progress because these slices complete only selected portions of those tasks.
