...
The first `@recruitos/runtime` slice now contains the pinned SQLite and Drizzle dependencies, a typed connection lifecycle, verified connection PRAGMAs, and a neutral strict migration-smoke table. Product schema and migrations, applications, CI, rubric-specific implementation, product fixtures, eval implementation, product tests, and product UI do not exist yet. T1, T2, and T3 remain in progress because these slices complete only selected portions of those tasks.
